...
namespace ClientReservasi_071
{
    public partial class Register : Form
    {
        ...
        {
            string username = textBoxUsername.Text;
            DialogResult dialogResult = MessageBox.Show("Apakah anda ingin menghapus data ini?",
                "Hapus Data", MessageBoxButtons.YesNo);

            if (dialogResult == DialogResult.Yes)
            {
                string a = service.DeleteRegister(username); // Mengambil return value service
                MessageBox.Show(a);
                Clear();
                TampilData();
            }
